...
export function CartLink({ locale }: { locale: string }) {
  ...
  return (
    ...
    >
      ...
      {displayCount > 0 ? (
        <span className="absolute -right-0.5 -top-0.5 flex h-4 min-w-4 items-center justify-center rounded-full bg-saffron px-1 text-[10px] font-bold text-foreground">
          {displayCount}
        </span>
      ) : null}
    </Link>
